            ...
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd
                  ...
                  @click="
                    changeActive(saleAttr.spuSaleAttrValueList, saleAttrValue)
                  "
                >
...
export default {
  ...
  methods: {
    // 属性值类名设置 参数一：数组 里面包含所有对象属性 参数二：点击的那个对象属性
    changeActive(saleAttr, saleAttrValue) {
      // 遍历数组
      saleAttr.forEach((item) => {
        // isChecked属性控制类名是否添加 当属性值为0即不添加，为1则添加类名
        // 排他思想 将数组里每一个对象身上的类名去掉 --- isChecked=0
        item.isChecked = 0;
      });
      // 将点击的属性的加上类名 --- isChecked=1
      saleAttrValue.isChecked = 1;
    },
    // 对输入框的值进行限制 只能为正整数且不为0
    changeSkuNum(e) {
    ...
    async addShopcar() {
      try {
        ...
        // 当传递的参数是一个大型参数（对象）时，使用路由传参会导致地址栏显示不认识的字符串
        // 可以使用本地存储或者会话存储
